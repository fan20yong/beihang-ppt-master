...
description: >
  Beihang-adapted AI-driven multi-format SVG content generation system. Converts source documents
  (PDF/DOCX/URL/Markdown) into high-quality SVG pages and exports to PPTX through
  multi-role collaboration with bundled privacy-safe Beihang University templates.
  Use when user asks to "create PPT", "make presentation", "生成PPT", "做PPT",
  "制作演示文稿", mentions "ppt-master", "beihang-ppt-master", "北航PPT", or needs
  Beihang/BUAA academic report slides.
---

# Beihang PPT Master Skill

> Beihang-adapted AI-driven multi-format SVG content generation system. Converts source documents into high-quality SVG pages through multi-role collaboration and exports to PPTX.

> Beihang addition: use `${SKILL_DIR}/templates/decks/beihang_university` when the user asks for Beihang / BUAA / 北航 academic report style. Privacy-safe native PPTX master assets live in `${SKILL_DIR}/templates/native_pptx/beihang/`; they contain masters/layouts/themes only, not source report slides.
...
| Beihang native PPTX masters | `${SKILL_DIR}/templates/native_pptx/beihang/` | Privacy-safe master/layout PPTX files for native template-fill or manual reuse |
